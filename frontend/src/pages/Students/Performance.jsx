...
const getStats = (performances) => {
  if (!performances.length)
    return { total: 0, avg: 0, max: 0, min: 0, subjects: 0 };
  ...
  useEffect(() => {
    const fetchPerformance = async () => {
      ...
      try {
        ...
        const studentPerformances = allPerformances.filter((perf) => {
          const studentEmail = (
              (perf.student.email && perf.student.email) ||
              (perf.email && perf.email)
            )
          if(!studentEmail) {
              return false
          }

          return studentEmail.trim().toLowerCase() === studentInfo.email.trim().toLowerCase()
        });
              ...
              >
                ...
                {performanceData.length > 0 ? (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: "#6b7280" }}>
                      Chart will appear when performance data is available
                    </div>
                  </div>
                )}
              </div>
                    ...
                    >
                      ...
                      >
                        <div
                          style={{
                            width: `${
                              (perf.score / Math.max(...scores)) * 100
                            }%`,
                            height: "100%",
                            background:
